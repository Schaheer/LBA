...
//            customerProfiledID: DataCacheManager.shared.loadRegisterVerifyOTPResponse()?.consumerList?.first?.rdaCustomerProfileID,
            ...
            rdaCustomerProfileId: DataCacheManager.shared.loadRegisterVerifyOTPResponse()?.consumerList?.last?.rdaCustomerProfileID, reasonForVisaDebitCardRequestId: serviceChannelsVM.getReasonSelected(), mailingAddrPrefId: serviceChannelsVM.getDeliverySelected(), esoaInd: SOASwitch.index == 1 ? true.intValue : false.intValue
        )
    }
